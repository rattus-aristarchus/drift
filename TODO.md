...
возможно, от этого удастся избавиться). и это поможет реорганизовать выполнение: вместо того чтобы 
обходить всю карту и вызывать индивидуально для каждого ресурса/популяции их эффекты можно обходить 
behaviors; так обеспечим что методы будут вызываться не вперемешку (а б в для популяции а, а б в для 
популяции б) а строго по порядку (а для популяции а и б, б для популяции а и б и т.д.).
- эффекты точно надо сделать объекты - держать factory статической переменной вообще не дело
...
- разные геттеры из effects_util. скорее их стоит распихать по классам, 
потому что сейчас каждый раз возникает путаница на тему того что мы ищем в классе, 
а что в effects_util
...
- труд надо сделать отдельным ресурсом, привязанным к популяции
...
- формы зависимости: собственность на труд (через структуру - типо государства или барщины); собственность на землю 
(через структуру - аренда); труд по найму (через структуру - рынок труда)
- отрефакторить функцию для производтсвенных цепочек; возможно, тут стоит создать объект Production
по формату объекта Market, который будет рассчитывать производственные цепочки
...
## верхний уровень, область духа

нижний уровень - популяции; верхний уровень - уникальные явления (например, произведения искусства
или крупные инженерные сооружения типо плотин). генерируются случайно, вероятность определяется
свойствами популяций (напр. больше класс грамотных аристократов - больше шанс на какое-нибудь стихотворение
и т.п.)
